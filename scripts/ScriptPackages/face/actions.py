...
def update_second(face=None):
    ...
    for inverse in pm.ls("*Inverse", type="transform"):
        ...
        if second.t.inputs(type="plusMinusAverage"):
            ...
            plus = pm.createNode("plusMinusAverage", n=n + "OffsetPlus")
            plus.output3D.connect(second.t, f=1)
            follicle.outTranslate.connect(plus.input3D[0])
        plus.input3D[1].set(point-follicle.outTranslate.get())
